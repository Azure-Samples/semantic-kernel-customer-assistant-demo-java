...
public class CustomerRoutes {
    ...
    public Uni<QueryResult> receiveMessage(
        ...
        if (chat.getLastMessage().isPresent() && chat.getMessages().size() > 2) {
            ...
            Mono<QueryResult> response = kernel
                    ...
                    .collectList()
                    .map(it -> {
                        return new QueryResult(String.join("\n", it), customer.getUid());
                    });
    ...
    private Mono<QueryResult> executeQuery(String query, Customer customer, ChatHistory chatHistory) {
        chatHistory.addUserMessage(query);
        return classifyStatement(query)
                .flatMap(type -> executeRequest(customer, query, type, chatHistory))
                .handle((it, sink) -> {
                    try {
                        String msg = new ObjectMapper().writerWithDefaultPrettyPrinter().writeValueAsString(it);
        ...
        switch (type) {
            ...
            case FACT -> {
                return customerController.addCustomerFact(customer, request)
                        .then(Mono.just(new QueryResult("Recorded note: statement", new Documents(customer.getUid(), List.of()))));
            }
            case EVENT -> {
                return customerController.addLogEvent(customer, request)
                        .then(Mono.just(new QueryResult("Recorded event", new Documents(customer.getUid(), List.of()))));
            }
            default -> {
                return Mono.just(new QueryResult("Unknown statement type", new Documents(customer.getUid(), List.of())));
            }
        }
    }
